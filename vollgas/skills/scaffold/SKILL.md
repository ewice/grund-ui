---
name: "scaffold"
description: "Use after /component-spec to create directory structure, types.ts, context interfaces, and barrel exports. Validates structure only — reviewer gates run via vollgas:review-gate during implementation. Next step after /component-spec."
---
...
element implementation — better to surface the gap now.
...
4. **If any required controller is missing — STOP. Do not create any directories or files.** Output:
   ...
   Build each missing controller first (plan it with vollgas:writing-plans, guided by
   vollgas/refs/component-shapes.md and the relevant refs/ documents).
...
**Next step: `vollgas:writing-plans` to draft the implementation plan (engine → elements → stories), then `vollgas:subagent-driven-development` to execute it.**
