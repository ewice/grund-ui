...
description: "Use when a bug or pattern issue may affect multiple components. Dispatches one subagent per component to check for the same issue. Returns consolidated findings. Called from bug-fix flows and /update-dependency, or invoked directly."
---
...
### Affected
- `accordion`: src/components/accordion/root/accordion.ts:42 — {description}
- `tabs`: src/components/tabs/trigger/tabs.ts:17 — {description}
...
If invoked from a bug-fix flow or `/update-dependency`: return findings to the caller for fixing.
