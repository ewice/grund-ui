...
- `BLOCKER` — correctness risks, invariant violations, must fix before shipping
- `OBSERVATION` — complexity or style improvements, worth noting but not a gate
...
#### Finding 1: Ambiguous state ownership
- Severity: BLOCKER
- File: src/components/toggle-group/toggle-group.ts:102
- Issue: Selected value is mutated in both the host and the group context object, leaving no single source of truth
- Evidence: `code-quality#H1` — both host and context write to selectedValue with different guard logic
- Suggested fix direction: Pick one owner for selection state and make the other layer query or derive from it
...
One finding per issue. Evidence must cite the rule/principle and include code or reference that proves the issue. No patches — fix direction only.
