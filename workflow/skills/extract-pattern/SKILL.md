...
Write `src/controllers/{name}.ts` (for shared reactive controllers or engines) or `src/utils/{name}.ts` (for pure utilities):
- **Reactive Controller:** implement `ReactiveController`, call `host.addController(this)`, add `hostConnected()` / `hostDisconnected()` lifecycle
- **Engine:** plain class, no `ReactiveController`, no `host` argument, no lifecycle hooks — independently testable with `new Engine()`
- **Utility:** pure function(s), no class, no state
...
Read `workflow/reviewers/lit-reviewer/SKILL.md`. Use its content as the Agent prompt. Dispatch as Agent call. Read and inject as context: new shared file content, all updated consumer file contents, `workflow/refs/lit-patterns.md` content, `workflow/refs/ssr-contract.md` content.
