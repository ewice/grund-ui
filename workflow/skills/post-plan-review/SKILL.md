...
2. **Change types** — read the task titles and classify using the keyword-to-change-type mapping in `workflow/refs/reviewer-dispatch.md`. A plan may have multiple change types.
...
Use the change-type selection table in `workflow/refs/reviewer-dispatch.md` to determine which reviewers to run.

### Phase 3 — Run reviewers (parallel)

Read `workflow/refs/reviewer-dispatch.md` for context injection rules. Read each selected reviewer's SKILL.md from `workflow/reviewers/{name}/SKILL.md`. Use its content as the Agent prompt. Dispatch all selected reviewers as simultaneous Agent calls, injecting context per the dispatch table.

### Phase 4 — Patch loop

Fix all blockers. Follow the patch loop rules in `workflow/refs/reviewer-dispatch.md` — re-run only the reviewer(s) that flagged blockers, not the full fleet.
