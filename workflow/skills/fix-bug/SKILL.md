...
Read `workflow/refs/reviewer-dispatch.md` for the change-type selection table and context injection rules. Classify the bug fix by change type (typically "Bug fix", but use "Accessibility or keyboard change" if the fix is ARIA/focus-related, etc.). Select reviewers per the dispatch table.
