...
**Location:** `vollgas/skills/build-controller/SKILL.md`
...
Read the controller spec from `vollgas/refs/component-shapes.md`. Then read the relevant domain reference doc:

| Controller domain | Reference doc |
|---|---|
| Focus (trap, restoration, roving, virtual) | `vollgas/refs/focus-management.md` |
| Form (internals, validation) | `vollgas/refs/form-participation.md` |
| Overlay (presence, positioning, scroll-lock, outside-click) | `vollgas/refs/component-shapes.md` (overlay section) |
| Feedback (live region) | `vollgas/refs/component-shapes.md` (feedback section) |
...
Dispatch 3 reviewers in parallel as Agent calls. For each, read its `vollgas/reviewers/{name}/SKILL.md` and use the content as the Agent prompt.

| Reviewer | Context to inject |
|---|---|
| `lit-reviewer` | Controller file, test file, `vollgas/refs/lit-patterns.md`, `vollgas/refs/ssr-contract.md` |
| `code-quality-reviewer` | Controller file, test file |
| `security-reviewer` | Controller file, test file |

This matches the "Internal (controller, context, lifecycle)" row in `vollgas/refs/reviewer-dispatch.md`.

Fix all blockers. Follow the patch loop rules in `vollgas/refs/reviewer-dispatch.md` (max 2 iterations per reviewer, then `/diagnose-failure`).
...
- `vollgas/refs/component-shapes.md` lists all planned controllers by component category
