...
3. Check: does a plan exist in docs/superpowers/plans/ for this work?
...
Plan: docs/superpowers/plans/2026-03-20-accordion-loop-focus.md (validated, no gaps)
...
**Invokes:** `superpowers:brainstorming` — explores intent, requirements, reference
...
The orchestrator reads `workflow/refs/reviewer-dispatch.md` and classifies the changes by
...
Unchanged from current `superpowers:finishing-a-development-branch`:
- Commit
- Branch management
- PR decision (human)
...
**Location:** `workflow/skills/orchestrate/SKILL.md` — a workflow skill, not a superpowers
skill. This allows the project to decouple from Superpowers in the future. The orchestrator
...
| Brainstorm | Subagent (invokes superpowers:brainstorming) |
...
|---|---|
| `superpowers:brainstorming` | Invoked by the **Brainstorm stage** — unchanged |
...
| `/smallest-diff` | Called by orchestrator in Review stage (before and after patch loop) |
...
| `superpowers:finishing-a-dev-branch` | Becomes the **Finish stage** — unchanged |
...
- Create `workflow/skills/orchestrate/SKILL.md` with Evaluate + Plan logic
...
- Update `workflow/refs/workflow-guidelines.md` to reference the orchestrator
- Update `workflow/refs/reviewer-dispatch.md` to remove per-skill references

---

## Resolved Decisions

1. **Orchestrator is a workflow skill** (`workflow/skills/orchestrate/SKILL.md`), not a
   superpowers skill. This decouples the pipeline from Superpowers and allows future
...
6. **Brainstorming uses `superpowers:brainstorming`.** Design exploration for new complex
